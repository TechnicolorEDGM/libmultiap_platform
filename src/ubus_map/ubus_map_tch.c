...
#define MODNAME		"libubus_map_tch"
...
static struct ubus_context *connect = NULL;
static int
...
static int
ubus_lua_connect_tch(lua_State *L)
{
	struct ubus_lua_connection *c;
	int timeout = luaL_optint(L, 2, 30);
	...
	connect = (struct ubus_context *)ctx;
	if (connect == NULL)
	{
		lua_pushnil(L);
		lua_pushstring( L, "CONNECT NULL" );
		return 2;
	}
...
ubus_lua__gc(lua_State *L)
{
	...
	if (c->ctx != NULL)
	{
		/* This check is added to see if the UBUS connect happened with an
		existing context in case of monitor thread, in which case it should not be freed
		Rather the free will be done by monitor thread */
		if(connect == NULL)
		{
		ubus_free(c->ctx);
		}
...
static const luaL_Reg ubus[] = {
	...
	{ "connecttch", ubus_lua_connect_tch },
...
int
luaopen_libubus_map_tch(lua_State *L)
{
	/* create metatable */
